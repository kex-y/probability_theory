...
lemma ae_eq_of_abs_sub_ae_eq {f g : α → ℝ} 
...
measure.trim_absolutely_continuous h $ @with_density_absolutely_continuous _ 𝓕 μ f
...
classical.some 
  (@signed_measure.exists_with_density_of_absolute_continuous _ _ 
    ...
    (@measure_theory.finite_measure_trim _ _ _ _ h $ 
      @finite_measure_with_density α 𝓕 _ _ hf₂)
    (measure.trim_absolutely_continuous h $ 
      @with_density_absolutely_continuous α 𝓕 π f)) 
...
(exists_prop.1 (classical.some_spec 
  (@signed_measure.exists_with_density_of_absolute_continuous _ _ 
    ...
    (@measure_theory.finite_measure_trim _ _ _ _ h $ 
      @finite_measure_with_density α 𝓕 _ _ hf₂)
    (measure.trim_absolutely_continuous h $ 
      @with_density_absolutely_continuous α 𝓕 π f)))).1
...
(exists_prop.1 (classical.some_spec 
  (@signed_measure.exists_with_density_of_absolute_continuous _ _ 
    ...
    (@measure_theory.finite_measure_trim _ _ _ _ h $ 
      @finite_measure_with_density α 𝓕 _ _ hf₂)
    (measure.trim_absolutely_continuous h $ 
      @with_density_absolutely_continuous α 𝓕 π f)))).2.symm
...
lemma condition_on_add : 
  condition_on h π (f + g) 
    (@measurable.add _ α _ _ 𝓕 _ _ _ hf₁ hg₁) 
...
lemma condition_on_smul (r : ℝ≥0∞) (hr : r < ∞) : 
  condition_on h π (r • f) 
    (@measurable.const_smul _ _ _ _ _ _ 𝓕 _ _ hf₁ r)
...
lemma lintegral_condition_on_eq : 
  ∫⁻ x, condition_on h π f hf₁ hf₂ x ∂(π.trim h) = 
  ∫⁻[𝓕] x, f x ∂π :=
begin
  rw [← lintegral_univ_eq, condition_on_spec', lintegral_univ_eq],
